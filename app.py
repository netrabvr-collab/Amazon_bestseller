...
def hybrid_recommend(book_name):
    ...
    for rec in recommended:
        ...
        author = books.loc[books["title"] == title, "authors"].values[0]

        print(f"{title} by {author}")

        if shared_tags:
            print("Reason: shared tags →", ", ".join(shared_tags))
        else:
            print("Reason: liked by similar readers")

        print("Hybrid score:", round(score,3))
        print()
# =========================
# RUN
# =========================
